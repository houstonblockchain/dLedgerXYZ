...
const ItemList = {
  oninit (vnode) {
    ...
    const refresh = () => {
      api.get('records?recordType=item').then((records) => {
        ...
        m(Table, {
          ...
          rows: vnode.state.filteredRecords.slice(
            ...
            (vnode.state.currentPage + 1) * PAGE_SIZE)
                .map((rec) => [
                  m(`a[href=/items/${rec.recordId}]`, {
                    oncreate: m.route.link
                  }, truncate(rec.recordId, { length: 32 })),
                  getPropertyValue(rec, 'itemName'),
                  getPropertyValue(rec, 'itemType'),
                  getPropertyValue(rec, 'quantity'),
                  getPropertyValue(rec, 'unitOfMeasure'),
                  getPropertyValue(rec, 'itemCategory'),
