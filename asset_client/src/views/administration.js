...
const _labelProperty = (label, value) => [
  m(".list-group",
    m("a.list-group-item.list-group-item-action.flex-column.align-items-start.active[href='#']",
      [
        m(".d-flex.w-100.justify-content-start",
          m("h5.mb-1", label)
        ),
        m("p.mb-1", value)
      ]
...
const _propLink = (record, propName, content) =>
  m(`a[href=/facilities/${record.recordId}/${propName}]`,
    { oncreate: m.route.link },
    content)
...
const FacilityDetail = {
  ...
  view(vnode) {
    ...
    return [
      m('.facility-detail',
        m('h1.text-left.text-primary', record.recordId),
        _row(
        ...
        _row(
          _labelProperty('Admin', _agentLink(custodian)),
          m(TransferControl, {
            publicKey,
            record,
            agents: vnode.state.agents,
        ...
        _row(
          _labelProperty('Business name', getPropertyValue(record, 'legalName')),
          _labelProperty('License type', getPropertyValue(record, 'licenseType'))),

        _row(
          _labelProperty(
            'DBA',
            _propLink(record, 'dba', _formatValue(record, 'dba'))
          ),
          (isReporter(record, 'dba', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'dba',
        ...
        _row(
          _labelProperty(
            'Status',
            _propLink(record, 'licenseStatus', _formatValue(record, 'licenseStatus'))),
          (isReporter(record, 'licenseStatus', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'licenseStatus',
        ...
        _row(
          _labelProperty(
            'Facility image',
            _propLink(record, 'licenseImage', _formatValue(record, 'licenseImage'))),
          (isReporter(record, 'licenseImage', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'licenseImage',
        ...
        _row(
          _labelProperty(
            'Email',
            _propLink(record, 'licenseEmail', _formatValue(record, 'licenseEmail'))),
          (isReporter(record, 'licenseEmail', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'licenseEmail',
                label: 'Email',
        ...
        _row(
          _labelProperty(
            'Phone number',
            _propLink(record, 'licensePhoneNumber', _formatValue(record, 'licensePhoneNumber'))),
          (isReporter(record, 'licensePhoneNumber', publicKey) && !record.final
            ? m(ReportValue,
              {
        ...
        _row(
          _labelProperty(
            'Address',
            _propLink(record, 'licenseAddress', _formatValue(record, 'licenseAddress'))),
          (isReporter(record, 'licenseAddress', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'licenseAddress',
                label: 'Address',
        ...
        _row(
          _labelProperty(
            'UIDs',
            _propLink(record, 'uids', _formatValue(record, 'uids'))),
          (isReporter(record, 'uids', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'uids',
                label: 'UIDs',
        ...
        _row(
          _labelProperty(
            'Titles',
            _propLink(record, 'titles', _formatValue(record, 'titles'))),
          (isReporter(record, 'titles', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'titles',
                label: 'Titles',
        ...
        _row(
          _labelProperty(
            'Strains',
            _propLink(record, 'strains', _formatValue(record, 'strains'))),
          (isReporter(record, 'strains', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'strains',
                label: 'Strains',
        ...
        _row(
          _labelProperty(
            'Items',
            _propLink(record, 'items', _formatValue(record, 'items'))),
          (isReporter(record, 'items', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'items',
                label: 'Items',
        ...
        _row(
          _labelProperty(
            'Rooms',
            _propLink(record, 'rooms', _formatValue(record, 'rooms'))),
          (isReporter(record, 'rooms', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'rooms',
                label: 'Rooms',
        ...
        _row(
          _labelProperty(
            'Patients',
            _propLink(record, 'patients', _formatValue(record, 'patients'))),
          (isReporter(record, 'patients', publicKey) && !record.final
            ? m(ReportValue,
              {
                name: 'patients',
                label: 'Patients',
...
const _formatValue = (record, propName) => {
  ...
  if (prop) {
    return parsing.stringifyValue(prop, '***', propName)
  } else {
    return 'N/A'
  }
}
