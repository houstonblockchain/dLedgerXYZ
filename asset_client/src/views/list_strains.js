...
const StrainList = {
  oninit (vnode) {
    ...
    const refresh = () => {
      api.get('records?recordType=strain').then((records) => {
        ...
        m(Table, {
          ...
          rows: vnode.state.filteredRecords.slice(
            ...
            (vnode.state.currentPage + 1) * PAGE_SIZE)
                .map((rec) => [
                  m(`a[href=/strains/${rec.recordId}]`, {
                    oncreate: m.route.link
                  }, truncate(rec.recordId, { length: 32 })),
                  getPropertyValue(rec, 'testingStatus'),
                  getPropertyValue(rec, 'thcInput'),
                  getPropertyValue(rec, 'cbdInput'),
                  getPropertyValue(rec, 'indicaInput'),
                  getPropertyValue(rec, 'sativaInput'),
                  getPropertyValue(rec, 'rodellisInput'),
