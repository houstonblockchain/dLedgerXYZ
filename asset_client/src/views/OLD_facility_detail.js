...
const authorizableProperties = [
  ...
  //TODO: ADDRESS IMAGE INPUT AND SAVE ['licenseImage', 'Image'],
  ['email', 'Email'],
...
const _labelPropertyEase = (label, value) => [
  m('dl',
    m('dt', label),
    m('dd', value))
]
...
const TransferDropdown = {
  view(vnode) {
    ...
    return [
      m('.dropdown',
        m('button.btn.btn-block.dropdown-toggle.text-left',
...
const FacilityDetail = {
  ...
  view(vnode) {
    ...
    const _labelProperty = (label, value) => 
      ...
      m(".list-group-item.list-group-flush",
        [
          ...
          m("p.mb-1", value),
          /*
            (isReporter(record, propName, publicKey) && !record.final
            ? m(ReportValue,
              {
                name: value,
                label: 'Enter' + ' ' + label,
                record,
                typeField: typesValue,
                type: payloads.updateProperties.enum.types,
                xform: (x) => x,
      ...
      const _labelAddressProperty = (label, value) => 
    ...
    return [
      m('.facility-detail',
        ...
        _row(
          _labelPropertyEase('Admin', _agentLink(custodian)),
          m(TransferControl, {
            publicKey,
            record,
            agents: vnode.state.agents,
        ...
        _row(
          _labelPropertyEase('Business name', getPropertyValue(record, 'legalName')),
          _labelPropertyEase('License type', getPropertyValue(record, 'licenseType'))),
        ...
        m(".list-group",
          [
            ...
            _labelProperty(
              'License Category',
              _propLink(record, 'licenseCategory', _formatValue(record, 'licenseCategory'))//,
              //"stringVAlue",
              //"STRING"
            ),
            ...
            _labelProperty(
              'License Expiration',
              _propLink(record, 'licenseExpiration', _formatValue(record, 'licenseExpiration'))//,
              //"stringVAlue",
              //"STRING"
            ),
            /**
            ...
            TODO:  Need to implement an image */
            _labelProperty(
              'Email',
              _propLink(record, 'email', _formatValue(record, 'email'))//,
              //"stringVAlue",
              //"STRING"
            ),
            _labelProperty(
              'Phone Number',
              _propLink(record, 'phoneNumber', _formatValue(record, 'phoneNumber'))//,
              //"stringVAlue",
              //"STRING"
            ),
            _labelAddressProperty(
              'Address',
              _propLink(record, 'address', parsing.stringToArray(getPropertyValue(record, 'address'), ';....'))//,
              //"stringVAlue",
              //"STRING"
            ),
            _labelProperty(
...
/**
 * 
 * @param {Return the record's value as a formatted value} record 
 * @param {*} propName 
 */
const _formatStringtoAddress = (record, propName) => {
  let prop = getPropertyValue(record, propName)
  if (prop) {
    return stringToArray(prop);
  } else {
    return 'N/A'
  }
}
